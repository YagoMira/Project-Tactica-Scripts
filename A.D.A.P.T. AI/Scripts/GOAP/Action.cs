...
namespace ADAPT
{
    public abstract class Action : MonoBehaviour
    {
        ...
        public bool addedPreviously = false; //In case of the elements (Preconditions/Effects) has been created previously by the user on the Preconditions/Effects lists
        ...
        void PerformData() //Store public preconditions and effects list into preconditions and effects dictionaries.
        {
            ...
            if (preconditions_list != null)
            {
                foreach (ResourceStruct r_p in preconditions_list)
                {
                    ...
                    if (!preconditions.ContainsKey(r_p.key))
            ...
            if (effects_list != null)
            {
                foreach (ResourceStruct r_e in effects_list)
                {
                    ...
                    if (!effects.ContainsKey(r_e.key))
        ...
        public void AddPreconditionToList(ResourceStruct resource)
        {
            if (!preconditions_list.Contains(resource))
                preconditions_list.Add(resource);
        }

        public void AddEffectToList(ResourceStruct resource)
        {
            if (!effects_list.Contains(resource))
                effects_list.Add(resource);
        }
